...
- 🔗 **点号对齐 + 勾选参与计算的点**：机器人程序按点名（`POS07` → `P7`）、三坐标文件按首列点号（`P7,…`）自动匹配，两侧顺序不一致也能对上；配对表里可逐点勾选参与计算，用于剔除异常点后重算（与收费软件一致）
...
| 选项 | 含义 | 模型 |
|---|---|---|
| 自动判断 | 有姿态且工具 Z 轴跨度 ≥ 5° → 走 TCP 模型，否则走单一刚体变换 | 自动 |
| 机器人点 = 电极帽触点 | `.dat` 里的点已经是电极帽实际触点（TCP 已标定好） | `rob ≈ R·base + t` |
